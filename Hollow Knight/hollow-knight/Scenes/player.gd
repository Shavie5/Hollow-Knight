...
func _physics_process(delta: float) -> void:
	...
	# Manejar ataques
	if Input.is_key_pressed(KEY_D) and not is_attacking and attack_timer <= 0:
		is_attacking = true
		attack_timer = ATTACK_COOLDOWN  # Inicia el enfriamiento
		jump_state = "attack"
		$AnimatedSprite2D.play("attack")  # Reproducir animación de ataque

	# Detectar fin de la animación de ataque
	if is_attacking and $AnimatedSprite2D.animation == "attack":
		if $AnimatedSprite2D.frame == $AnimatedSprite2D.sprite_frames.get_frame_count("attack") - 1:
			is_attacking = false
			if not is_on_floor():
				$AnimatedSprite2D.play("jump")
			else:
				$AnimatedSprite2D.play("idle")  # Animación cuando termina el ataque en el aire
	...
	is_moving = false
	if not dash_state and not is_attacking:
	...
	if is_on_floor() and not is_moving and jump_state == "idle" and $AnimatedSprite2D.animation != "recover_dash" and not is_attacking:
		$AnimatedSprite2D.play("idle")
